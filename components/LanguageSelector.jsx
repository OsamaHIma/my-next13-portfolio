  ...
  return (
    <Menu className={`relative inline-block`}>
      <MenuHandler>
        ...
        >
          <Languages size={25} />
          <span className="ltr:mr-2 rtl:ml-2">{selectedLanguage.name}</span>
      ...
      <MenuList
        dir="ltr"
        className="overflow-y-auto max-h-80 bg-stone-200 dark:bg-main-color border-0"
      >
        {languages.length &&
          languages.map((language, index) => (
            <MenuItem
              key={index}
              className={`block my-1 w-full cursor-pointer z-50 px-4 py-2 text-left text-stone-800 dark:text-stone-100 transition-all dark:hover:!bg-theme-color/70 ${
